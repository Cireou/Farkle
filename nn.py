...
class NNPolicy:
    def __init__(self, name, verbose=False):
        ...
        self.win_prob_model = train_win_prob.load()
        self.action_model = train_action.load()
    ...
    def choose_dice(self, game, pos):
        ...
        max_prob_pos, keep = optimal_dp.scoring_decision_state(None, (b,d,n,t), roll, return_prob=False, model=self.win_prob_model)
    ...
    def choose_action(self, game, pos):
        ...
        """
        encoded_pos = encode_sample.normalize_key(','.join(str(n) for n in self.max_prob_pos))
        x_predict = np.matrix(encoded_pos)
        val = self.action_model.predict(x_predict)
        if val >= 0.50: # Currently NN doesn't return only 0 or 1 for action, we round
            self.action = 'reroll'
        else:
            self.action = 'bank'
